<template>
  <div
    class="min-h-screen flex items-center justify-center bg-gradient-to-br from-blue-100 via-indigo-100 to-purple-100 p-6"
  >
    <div
      class="max-w-md w-full bg-white rounded-2xl shadow-xl p-8 space-y-6 border border-gray-100"
    >
      <!-- Header -->
      <div class="text-center space-y-2">
        <div
          class="w-16 h-16 mx-auto bg-gradient-to-r from-indigo-500 to-blue-600 rounded-full flex items-center justify-center shadow-md"
        >
          <svg
      ...
      <form @submit.prevent="handleRegister" class="space-y-4">
        ...
        <div v-if="errors.general && errors.general.length > 0" class="bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded relative mb-4" role="alert">
            <span class="block sm:inline">{{ errors.general[0] }}</span>
        </div>
        <div>
        ...
        <button
          type="submit"
          :disabled="loading"
          class="w-full py-3 bg-gradient-to-r from-indigo-500 to-blue-600 text-white font-semibold rounded-lg hover:shadow-lg hover:scale-[1.02] transition-all duration-200 disabled:opacity-60 disabled:cursor-not-allowed"
        >
          <span
            v-if="loading"
      ...
      <p class="text-center text-sm text-gray-500">
        Sudah punya akun?
        <router-link to="/login" class="text-blue-800 hover:underline"
...
const handleRegister = async () => {
  ...
  // errors.value is cleared by useAuth before each request
  
  const success = await register({ ...form.value });
  if (success) {
    successMessage.value = 'Registrasi berhasil! Silakan masuk dengan akun baru Anda.';
